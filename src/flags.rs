...
pub fn parse_flags_and_filenames(
    mut args: Vec<String>,
) -> Result<(HashSet<Flag>, Vec<String>), String> {
...
fn parse_flag(arg: &str, flags: &mut HashSet<Flag>) -> Result<(), String> {
    for c in arg.chars().skip(1) {
        match c {
            'c' => {
                flags.remove(&Flag::Characters);
                flags.insert(Flag::Bytes);
            }
            'l' => {
                flags.insert(Flag::Lines);
            }
            'w' => {
                flags.insert(Flag::Words);
            }
            'm' => {
                flags.remove(&Flag::Bytes);
                flags.insert(Flag::Characters);
            }
...
mod tests {
    ...
    fn empty_args_returns_default_flags() {
        let (flags, files) = parse_flags_and_filenames(vec![]).unwrap();
    ...
    fn explicit_flags_override_defaults() {
        let (flags, _) = parse_flags_and_filenames(vec!["-l".into()]).unwrap();
        assert!(flags.contains(&Flag::Lines));
        assert!(!flags.contains(&Flag::Bytes));
        assert!(!flags.contains(&Flag::Words));
    }
    ...
    fn m_flag_replaces_c_flag() {
        let (flags, _) = parse_flags_and_filenames(vec!["-c".into(), "-m".into()]).unwrap();
    ...
    fn c_flag_replaces_m_flag() {
        let (flags, _) = parse_flags_and_filenames(vec!["-m".into(), "-c".into()]).unwrap();
        assert!(flags.contains(&Flag::Bytes));
        assert!(!flags.contains(&Flag::Characters));
    }
    ...
    fn files_collected_after_flags() {
        let (_, files) =
            parse_flags_and_filenames(vec!["-l".into(), "a.txt".into(), "b.txt".into()]).unwrap();
        assert_eq!(files, vec!["a.txt", "b.txt"]);
    }

    #[test]
    fn illegal_flag_returns_err() {
        assert!(parse_flags_and_filenames(vec!["-z".into()]).is_err());
    }
}
